...
import { useCategories, useCreateProduct } from "@/services/products";
...
export function Dashboard() {
  ...
  const form = useForm<ProductManagementData>({
  ...
  useEffect(() => {
    if (status == "success") navigate({ to: "/dashboard/products" });
  }, [status, navigate]);

  const onSubmit = async (data: ProductManagementData) => {
        ...
        <div className="flex flex-1 flex-row items-center gap-4 px-4">
          ...
          <div className="flex items-center gap-2 sm:ml-auto">
            <Button variant="outline" size="sm" asChild>
              <Link to="/dashboard/products">Discard</Link>
            </Button>
